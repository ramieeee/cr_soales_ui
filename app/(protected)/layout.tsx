        ...
        <div className="mx-auto grid w-full max-w-[1320px] gap-6 px-4 py-6 md:grid-cols-[240px_1fr] md:px-6">
          <aside className="rounded-2xl border border-white/15 bg-[rgba(18,18,18,0.8)] p-5">
            ...
            <nav className="mt-5 grid gap-2">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="rounded-xl border border-white/10 px-3 py-2 text-sm font-medium text-[#d8d8d8] transition-colors duration-150 ease-out hover:border-white/30 hover:bg-white/[0.04]"
                >
                  {item.label}
                </Link>
              ))}
            ...
            <form action="/api/auth/logout" method="post" className="mt-8">
              <button
                type="submit"
                className="w-full rounded-xl border border-white/15 bg-[rgba(20,20,20,0.9)] px-3 py-2 text-sm font-semibold text-[#e5e5e5] transition-colors duration-150 ease-out hover:border-white/30"
              >
                Logout
              </button>
            </form>
